...
import static com.example.medialab.hangmanExceptions.checkExceptions;

/* Java TextIO code from source:https://math.hws.edu/eck/cs124/javanotes6/source/TextIO.java
Modified for hangman project.
 */
...
public class TextIO {
    ...
    public static class SeparateTheWords {
        public static void separateWords(String tempFile) throws InvalidCountException {

            TextIO.readFile(tempFile);
            ...
            while (word != null) {
                ...
                if (!wordList.contains(word)) {
                    ...
                    if (word.length() > 6) {
                        //We are ignoring same words so this exception is not important.
                        //throw new InvalidCountException("The word count is not correct. Please try another book.");
                    }
                }
                word = readNextWord();
            }
            ...
            int percentage = supercount*100 / count;

            checkExceptions(count, percentage);
            System.out.println("\n\nDone.\n\n");
     ...
     * program, the list only contains words made up of upper case
